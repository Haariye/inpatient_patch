...
def _seed_nurse_shifts():
    """Create the three standard ward shifts (idempotent)."""
    shifts = [("Morning", "07:00:00", "14:00:00"),
              ("Afternoon", "14:00:00", "21:00:00"),
              ("Night", "21:00:00", "07:00:00")]
    for name, start, end in shifts:
        if not frappe.db.exists("Nurse Shift", name):
            try:
                d = frappe.get_doc({"doctype": "Nurse Shift", "shift_name": name,
                                    "shift_type": name, "start_time": start,
                                    "end_time": end})
                d.flags.ignore_mandatory = True
                d.insert(ignore_permissions=True)
            except Exception:
                frappe.log_error(frappe.get_traceback(), "seed nurse shifts")
...
EXAMPLE_PROTOCOLS = {
    "Orthopedic": {
        ...
        "forms": [
            ...
            ("Operation", "Clinical Procedure", 1, 10),
    ...
    "Obstetrics & Gynecology": {
        ...
        "forms": [
            ...
            ("Operation", "Clinical Procedure", 1, 8),
